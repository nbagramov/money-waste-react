...
function* workerLoadPurchases() {
  try {
    const data: IPurchase[] = yield axios.get('https://secret-hollows-04121.herokuapp.com/purchases/').then(res => res.data);
...
function* workerCreatePurchases({ payload }: CreatePurchaseAction) {
  try {
    ...
    if (!data.some((item) => item.id === payload.purchase.id)) {
      yield axios.post(`https://secret-hollows-04121.herokuapp.com/purchases/?=id${payload.purchase.id}`, {
    ...
    const newData: IPurchase[] = yield axios.get('https://secret-hollows-04121.herokuapp.com/purchases/').then(res => res.data);
...
function* workerUpdatePurchases({ payload }: EditPurchaseAction) {
  try {
    yield axios.patch(`https://secret-hollows-04121.herokuapp.com/purchases/?id=${payload.purchase.id}`, {
      id: payload.purchase.id,
      place: payload.purchase.place,
      price: payload.purchase.price,
      date: payload.purchase.date,
...
function* workerRemovePurchases({ payload }: DeletePurchaseAction) {
  try {
    yield axios.delete(`https://secret-hollows-04121.herokuapp.com/purchases/?id=${payload.id}`);
